...
/// Detect agent ID from .cmux/agent.json by walking up from CWD.
pub fn detect_agent_id() -> Option<String> {
    let mut dir = std::env::current_dir().ok()?;
    loop {
        let agent_file = dir.join(".cmux").join("agent.json");
        if agent_file.exists() {
            let data = std::fs::read_to_string(&agent_file).ok()?;
            let json: Value = serde_json::from_str(&data).ok()?;
            return json.get("agent_id").and_then(|v| v.as_str()).map(|s| s.to_string());
        }
        if !dir.pop() {
            break;
        }
